      ...
      try {
        ...
        if (message.channel.permissionsFor(message.member)
          ...
          if (room == undefined) {
            ...
            message.channel.send({ embed });
            client.channels.get('520188265618472960')
              .send(new Date().toLocaleString('en-US', { timeZone: 'America/New_York' }) + ` **${channelID} Channel registered for server ${message.guild.name} by ${message.author.username}**`);
...
exports.disconnectDiscord = function (globalid, settings) {
  ...
  try {
    client.channels.get(settings.link.channelid)
      .send({ embed });
  } catch (err) {
  }
};
...
exports.startOpTimer = function (global) {
  if (global.settings.link != undefined) {
    try {
      ...
      client.channels.get(global.settings.link.channelid)
        .send({ embed });
    } catch (err) {
    }
  }
